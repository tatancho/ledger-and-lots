      ...
      <div style={{ minHeight: '100vh', background: t.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'Inter, system-ui, sans-serif' }}>
        ...
        <div style={{ width: '100%', maxWidth: 460, animation: 'fadeUp 0.35s ease both' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 32 }}>
            {features.map(f => (
              <div key={f.label} style={{ background: t.bg2, border: '1px solid rgba(255,255,255,0.07)', borderRadius: 14, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 12 }}>
          ...
          <button onClick={onContinue} style={btnStyle}>Get started →</button>
    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: t.bg, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: 'Inter, system-ui, sans-serif' }}>
        ...
        <div style={{ width: '100%', maxWidth: 400, animation: 'fadeUp 0.35s ease both' }}>
          ...
          <div style={{ background: t.bg2, border: '1px solid rgba(255,255,255,0.07)', borderRadius: 20, padding: 28 }}>
            <form onSubmit={e => { e.preventDefault(); if (!name.trim()) return; onSubmit(name.trim()); }}>
              ...
              {name.trim() && (
                <div style={{ background: t.bg3, border: '1px solid rgba(255,255,255,0.07)', borderRadius: 12, padding: '14px 16px', display: 'flex', alignItems: 'center', gap: 12, marginBottom: 20 }}>
              ...
              <button
                ...
                style={{ ...btnStyle, opacity: saving || !name.trim() ? 0.6 : 1, cursor: saving || !name.trim() ? 'not-allowed' : 'pointer' }}
              >
          ...
          <p style={{ textAlign: 'center', marginTop: 16, fontSize: 11, color: t.text3, fontFamily: 'JetBrains Mono, monospace' }}>
            You can update this anytime in Profile and Security.
          </p>
        </div>
      </div>
    </>
